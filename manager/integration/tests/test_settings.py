...
def wait_for_toleration_update(core_api, apps_api, count,  # NOQA
    ...
    for _ in range(RETRY_COUNTS):
        ...
        if len(backing_images) != 0:
            pod_list = common.list_backing_image_manager_pods(core_api)
        else:
            pod_list = core_api.list_namespaced_pod(LONGHORN_NAMESPACE).items

        for p in pod_list:
            if len(backing_images) != 0:
                assert p.metadata.name.startswith("backing-image-manager")

            n_update = check_tolerations_set_update(p.spec.tolerations,
                                                    expected_tolerations,
                                                    chk_removed_tolerations)
            app_name = str(p.metadata.labels.get('app', ''))
            if app_name in skip_updates and n_update == 0:
                ok = True
            elif n_update == len(expected_tolerations):
                ok = True
        if ok:
            break
    assert ok
